const simFrag = `
    ...
    void main() {
        ...
        float circlarForce = 1.-smoothstep(0.3, 0.4, abs(pos.x-radius));
        float angle = atan(pos.y, pos.x)-info.y*0.1*mix(.5, 1.0, circlarForce);

        float targetRadius = mix(info.x, 1.8,0.5+0.45*sin(angle*2.0+uTime*0.5));
        radius += (targetRadius-radius)*mix(0.2, 0.5, circlarForce);        
        ...
        if (distToMouse > 0.0001) {
            ...
            pos.xy += 0.1 * dir * smoothstep(0.3, 0.0, distToMouse);
        }
